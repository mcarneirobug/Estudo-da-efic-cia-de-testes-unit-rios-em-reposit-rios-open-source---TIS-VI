...
\section{\esp Revisão da Literatura}
A partir do artigo \textit{A Comparative Study of the Effects of Pull Request on GitHub Projects} de \citeonline{7552027}, com o aumento da popularidade do GitHub houve um aumento no interesse em estudar o possível impacto dos \textit{pull requests} e na utilização dos recursos da ferramenta do Github no desenvolvimento de software. Além de hospedar repositórios de software, o GitHub também incorpora recursos de rede social, como seguir desenvolvedores e acompanhar projetos. Esses recursos de rede social ajudam os desenvolvedores a acompanhar seus projetos favoritos, as tendências de mercado e suas pessoas conhecidas. Eles também trazem um alto grau de transparência o que torna as ações e interações dos desenvolvedores mais visíveis e rastreáveis dentro e através do código aberto de projetos de software \cite{10.5555/2820518.2820563}.

\section{\esp Metodologia}

O objetivo da pesquisa é realizar a análise da eficácia dos testes unitários, com o objetivo de verificar a quantidade de bugs em repositórios do GitHub em relação a quantidade de testes. Tendo como intuito, fazer a análise de acordo com o ponto de vista dos desenvolvedores de software que tem como finalidade tornar os softwares mais robustos, gerando menos manutenção e correção de bugs.

A partir da pesquisa realizada pelo site Octoverse, do GitHub, foi possível analisar as linguagens com maior aderência no mercado. Além disso, buscamos relacionar as linguagens com os frameworks mais populares de testes unitários (Software Testing Help, 2021) chegando-se nas linguagens: Java, Python, JavaScript, C\#.

De acordo com Urlichs (2020), o Coveralls é um serviço da Web que permite aos usuários rastrear a cobertura de código de seu aplicativo ao longo do tempo para otimizar a eficácia de seus testes de unidade. O aplicativo gerencia os recursos associados em uma plataforma de Integração Contínua (CI) como Codefresh, para que o desenvolvedor consiga informações sobre a cobertura do teste automaticamente a cada construção de pipeline.

\subsection{\esp Questões de pesquisa}

\begin{enumerate} 
 \item [1.] A quantidade de bugs presentes no software diminui proporcionalmente à medida que a presença de testes unitários aumenta? ;
 \item [2.] Qual é a relação entre o code coverage e a quantidade de issues nas diferentes linguagens?;
 \item [3.] Entre as linguagens de programação dos repositórios buscadas, qual possui a maior quantidade de testes unitários?;
\end{enumerate}

\subsection{\esp Ferramentas}
Para a realização do experimento, utilizaremos as linguagens GraphGL e Python para a extração dos dados na API do GitHub e geração do arquivo .CSV. Além disso utilizaremos a badge do GitHub coverage que analisa as estatísticas de cobertura de código mais recentes em todos os seus repositórios, incluindo as porcentagens totais de cobertura e as linhas cobertas. 

\subsection{\esp Procedimento de coleta de dados}
O experimento será realizado a partir de três principais etapas: a coleta dos dados, a análise dos dados e a apresentação dos resultados. A coleta dos dados se iniciará através da criação de um script contendo o filtro das linguagens Java, Python, JavaScript e C\#. Inicialmente serão colhidos os 1000 primeiros repositórios possuindo no mínimo 10 bug issues abertas. Também serão filtrados apenas os reposit

órios que possuem a badge coverage no repositório. Serão colhidos, o nome do repositório, a quantidade de bug issues abertas, o valor presente na badge coverage, a idade do repositório e sua linguagem principal. Esses dados serão exportados para um arquivo .CSV.

O processo de análise será realizado visando responder às questões pesquisas. Dessa forma iremos analisar a média de bug issues por linguagem, a relação entre o code coverage e os bugs issues. Também serão analisados os repositórios que possuem o número acima e abaixo de 50\% de coverage e, por fim, as linguagens que possuem melhor relação code coverage / bug issues.

Para a apresentação dos resultados serão gerados gráficos que mostrem a tendência dos resultados além da utilização de recursos de medida de centralidade para demonstração dos resultados, como média, variância e desvio padrão. Os resultados serão analisados e será apresentada a conclusão do experimento.

% Figura
\begin{figure}[ht]
	\centering	
	\caption[\hspace{0.1cm}Grade Computacional.]{Processo de coleta de dados}
	\vspace{-0.4cm}
	\includegraphics[width=0.6\textwidth]{figuras/processo.jpg}
